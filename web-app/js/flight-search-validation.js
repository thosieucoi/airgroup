jQuery(document).ready(function(){
	...
	//Get the index of month 12
	var indexLastMonthOfCurrentYear;
	for(i=1;i<sortedMonthList.length+1;i++){
		if(sortedMonthList[i] == 12) {
			indexLastMonthOfCurrentYear = i;
		}
	}
	...
	for(i=1;i<sortedMonthList.length+1;i++){
		if(sortedMonthList[i-1]<10){
			// i (month) start with 1 -> (indexLastMonthOfCurrentYear + 2) instead of (indexLastMonthOfCurrentYear + 1)
			if(i < (indexLastMonthOfCurrentYear + 2)){
				jQuery('#omonth, #imonth').append('<option value="0'+sortedMonthList[i-1]+'">Tháng '+sortedMonthList[i-1]+'/' + currentYear + '</option>');
			} else {
				jQuery('#omonth, #imonth').append('<option value="0'+sortedMonthList[i-1]+'">Tháng '+sortedMonthList[i-1]+'/' + (currentYear + 1) + '</option>');
			}
		} else if(sortedMonthList[i-1]>9){
			if(i < (indexLastMonthOfCurrentYear + 2)){
				jQuery('#omonth, #imonth').append('<option value="'+sortedMonthList[i-1]+'">Tháng '+sortedMonthList[i-1]+'/' + currentYear + '</option>');
			} else {
				jQuery('#omonth, #imonth').append('<option value="'+sortedMonthList[i-1]+'">Tháng '+sortedMonthList[i-1]+'/' + (currentYear + 1) + '</option>');
			}
...
//			message = 'NgÃ y Ä‘i khÃ´ng Ä‘Æ°á»£c lÃ  ngÃ y trong quÃ¡ khá»©!'
//			isValid = 0
...
//		}
		...
		if(startSelectedMonth < currentMonth && desSelectedMonth < currentMonth && desSelectedMonth > 0 && desSelectedDate > 0){
			if(desSelectedMonth < startSelectedMonth){
				message = 'Ngày về phải lớn hơn ngày xuất phát!'
				isValid = 0
		...
		if(customerPhoneNumber != null && customerPhoneNumber != ""){
			if(customerPhoneNumber.length < 8){
				message = 'Số điện thoại phải là kí tự số và từ 8 đến 15 kí tự!'
				isValid = 0
			}
		}
		...
		jQuery('#validateMessage').css('color', '#F9B914').css('font-size', '13px').css('font-weight', 'bold');
