...
public class HongKongAirlinesFlight extends SearchFlights {
	...
	protected Response getResponse(CuriositySearch search) throws InterruptedException,
		...
		HongKongAirlinesRequestFirst requestFirst = new HongKongAirlinesRequestFirst();
		...
		HongKongAirlinesLanguage language = new HongKongAirlinesLanguage();
		Response languageResponse = language.getResponse(search);

		HongKongAirlinesRequest request = new HongKongAirlinesRequest();
		Response cookieResponse = request.getResponseWithCookies(
			search,
			languageResponse.getCookies());

		HongKongAirlinesProcess process = new HongKongAirlinesProcess();
		process.getResponseWithCookies(search, languageResponse.getCookies());

		HongKongAirlinesRequestResult requestResult = new HongKongAirlinesRequestResult();
		requestResult.getResponseWithCookies(search, languageResponse.getCookies());

		HongKongAirlinesResult result = new HongKongAirlinesResult();
