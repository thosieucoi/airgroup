...
public class AbacusFlight extends SearchFlights {
	...
	protected Response getResponse(CuriositySearch search) throws InterruptedException,
		...
		AbacusRequestFirst requestFirst = new AbacusRequestFirst();
		...
		AbacusLanguage language = new AbacusLanguage();
		Response languageResponse = language.getResponse(search);

		// System.err.println(languageResponse.getResponseBody());

		AbacusRequest request = new AbacusRequest();
		Response cookieResponse = request.getResponseWithCookies(
			search,
			languageResponse.getCookies());

		// System.err.println(cookieResponse.getResponseBody());

		AbacusProcess process = new AbacusProcess();
		process.getResponseWithCookies(search, languageResponse.getCookies());

		AbacusRequestResult requestResult = new AbacusRequestResult();
	...
	public List<CuriosityFare> getFare(CuriositySearch search) throws IOException,
		...
		for (Element flightElement : flightElements) {
			...
			String airlineCode = flightElement.select("img").first().attr("alt");
			...
			fare.setAirlineCode(airlineCode);
