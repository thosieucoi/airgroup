...
public class DeltaAirlinesFlight extends SearchFlights {
	...
	protected Response getResponse(CuriositySearch search) throws InterruptedException,
		...
		DeltaAirlinesRequestFirst requestFirst = new DeltaAirlinesRequestFirst();
		...
		DeltaAirlinesLanguage language = new DeltaAirlinesLanguage();
		Response languageResponse = language.getResponse(search);

		DeltaAirlinesRequest request = new DeltaAirlinesRequest();
		Response cookieResponse = request.getResponseWithCookies(
			search,
			languageResponse.getCookies());

		DeltaAirlinesProcess process = new DeltaAirlinesProcess();
		process.getResponseWithCookies(search, languageResponse.getCookies());

		DeltaAirlinesRequestResult requestResult = new DeltaAirlinesRequestResult();
		requestResult.getResponseWithCookies(search, languageResponse.getCookies());

		DeltaAirlinesResult result = new DeltaAirlinesResult();
	...
	public List<CuriosityFare> getFare(CuriositySearch search) throws IOException,
		...
		if (flightElements.size() == 0) {
			...
			search.createParam("cabin", "C");
			response = getResponse(search);
			doc = StringUtils.substringBetween(response.getResponseBody(), "<![CDATA[", "]]>");
			document = Jsoup.parse(doc);
			flightElements = document.select("div[class=flightResult]");
		}
		for (Element flightElement : flightElements) {
			...
			if (search.hasChildren()) {
				...
				fare.setPricePerChild((Float.parseFloat(childPrice) / search.getChildrenCount()) +
										(tax / search.getPassengersCount()));
			}

			// if (search.getInfantsCount() > 0) {
			// String infantPrice = priceElement
			// .select("tr:has(td:contains(Trẻ sơ sinh)) > td.textright")
			// .text()
			// .replaceAll(",", "");
			// }
			...
			fare.setTotalPrice(Float.parseFloat(totalPrice), search);
			fare.setAirlineCode("DL");
			fares.add(fare);
		}
	...
	private List<CuriositySegment> parseSegments(Element element, CuriositySearch search,
		...
		String cabin = (String) search.getParamValue("cabin");
		String[] segmentInfo;
		if (cabin.equals("C")) {
			segmentInfo = StringUtils
				.substringBetween(text, "</div> ", " <div>")
				.replaceAll("\n", "")
				.split("<br /> Business \\([A-Z]\\)<br /> ");
